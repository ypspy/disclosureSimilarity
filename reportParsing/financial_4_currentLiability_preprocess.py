...
df = pd.read_csv("financial_5_currentLiability.txt")
...
# 이상치 제거 1 길이가 7을 초과한 Entry 제거
df["drop"] = [True if len(x) > 7 else False for x in df["accounts"]]
df = df[df["drop"] == False]
del df["drop"]

# 산출치 입력
df["currentLiability"] = df["unit"].astype('int') * df["value"]
...
# # 산출치 추출
df = df[["key", "currentLiability"]]
df.to_csv("financial_5_currentLiability_preprocessed.txt")
