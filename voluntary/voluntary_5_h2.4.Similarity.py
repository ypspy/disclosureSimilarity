...
df = pd.read_csv("voluntary_3_h2.preprocessedDocumentsOut_FS.txt")

df["documents"].str.strip()  # Whitespace 제거
df["count"] = df["documents"].str.len()  # string 수 
df = df[df["count"] > 100]  # 입력내용이 100을 초과하는 입력값   

with open("voluntary_4_h2.tfIdfVec_twitter_noun_trigram.pkl", 'rb') as inp:
...
df["similarity"] = simList

dfYear = df[["key", "5", "10", "KSIC", "count", "similarity"]]
dfYear = dfYear[dfYear["similarity"] != 999]

dfYear.to_csv("voluntary_5_h2.similarity.txt")
